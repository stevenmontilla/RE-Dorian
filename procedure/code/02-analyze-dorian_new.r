...
<<<<<<< HEAD
dorianByHour <- ts_data(akflood, by="hours")
ts_plot(akflood, by="hours")
=======
dorianTweetsByHour <- ts_data(akflood, by="hours")
ts_plot(akflood, by="hours")
>>>>>>> upstream/main
...
akfloodNetwork <- network_graph(akflood, c("quote"))

plot.igraph(akfloodNetwork)
...
akfloodText = akflood %>% select(text) %>% plain_tweets()

# parse out words from tweet text
akfloodWords = akfloodText %>% unnest_tokens(word, text)
...
stop_words = stop_words %>%
  ...
  add_row(word="flooding",lexicon = "Search") %>%
  add_row(word="drainage",lexicon = "Search") %>%
  add_row(word="underwater",lexicon = "Search")%>%
  add_row(word="flash",lexicon = "Search")%>%
  add_row(word="tornado",lexicon = "Search")%>%
  add_row(word="tornadoes",lexicon = "Search") 

#delete stop words from dorianWords with an anti_join
akfloodWords =  akfloodWords %>% anti_join(stop_words)
...
akfloodWordPairs %>%
  ...
  labs(title = "Word Network of Tweets during Floods in late april/early may, 2021",
       x = "", y = "") +
  theme_void()
